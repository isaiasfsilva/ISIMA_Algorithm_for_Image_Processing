...
CImg<> G(imgIn.width(), imgIn.height());
CImg<> norm(imgIn.width(), imgIn.height());

cimg_forXY(imgIn,x,y)
{
   G(x,y) = sqrt(ImgInGrad[0](x,y)*ImgInGrad[0](x,y)+ImgInGrad[1](x,y)*ImgInGrad[1](x,y));
}
 ...
 int IterStop = 100;
  for (int n = 0; n < nbiter; ++n){
      ...
      float speed = 0;
      cimg_forXY((*LevelSet),i,j){
        float speed=0;// TO DO
        (*LevelSet)(i,j) = (*LevelSet)(i,j) + delta_t*speed;
      }
